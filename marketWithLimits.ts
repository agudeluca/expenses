...
const marketWithlimit = async ({
  ...
  const [markPrice, exchangeInfo] = await Promise.all([
    getMarkPrice(symbol),
    getSymbolExchangeInfo(symbol),
    getOrderBook(symbol),
  ]);
  let remainingQuantity = isCoin
    ? Number(quantity.toFixed(exchangeInfo?.quantityPrecision))
    : Number(
        (quantity / markPrice.markPrice).toFixed(
          exchangeInfo?.quantityPrecision
        )
      );
  ...
  while (remainingQuantity > 0) {
    ...
    const orderId: BigNumber | number = await createLimitOrder(
      symbol,
      price,
      remainingQuantity,
      side
    ...
    if (!orderId) {
      ...
      console.log(
        positionRiskAfter?.positionAmt,
        positionRiskBefore?.positionAmt
      );
    ...
    if (orderStatus.status === "FILLED") {
      ...
      const canceledOrder = await futuresCancel(
        symbol,
